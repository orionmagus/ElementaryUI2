...
const TransitionGrid = ({ visible, items, removeItem }) => {
  ...
  useChain(visible ? [containerRef, cardsRef] : [cardsRef, containerRef], [0, visible ? 0.1 : 0.6]);
        ...
        ({ item, key, props: { x, opacity } }) =>
          item && (
            <animated.div
              ...
              {cardsTransition.map(({ item, key, props: { translateY, opacity } }) => (
                <animated.div
                  className="card"
                  key={key}
                  style={{
                    opacity,
                    transform: translateY.interpolate(s => `translateY(${s}px)`)
                  }}
